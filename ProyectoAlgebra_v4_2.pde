...
void draw() {
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    if(!NoItem2)
    {
      triangle(ItemX[1], ItemY[1], ItemX[1]-20.0, ItemY[1]+30.0, ItemX[1]+20.0, ItemY[1]+30.0);
    }
    
    if (!NoItem3)
    {
      ellipse(ItemX[2], ItemY[2], 30.0, 30.0);
    }
  ...
  if (FinalFight)
  {
    ...
    fill(255,0,128);
    rect(40.0, height - 70.0, largoRect, 40.0);
      ...
      if(mouseX + radio > xBoss && mouseY + radio > yBoss && xBoss + medidaXboss > mouseX - radio && yBoss + medidaYboss > mouseY - radio)
    {
      ...
      if (millis() > miliseconds + 1000 && !GameOver)
      {
          miliseconds += 1000;
          timeFinalFight--;
    ...
    if (winFinalBoss)
    {
      ...
      textSize(30);
      fill(255, 0, 0);
      text("HE COULDN'T STORE ALL THAT POWER...", width / 2.0 - 270.0, 300.0);
